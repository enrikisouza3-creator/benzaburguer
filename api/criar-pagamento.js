// api/criar-pagamento.js -- Backend Benza Burguer x Mercado Pago
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const payerEmail = (email && email !== process.env.MP_ACCOUNT_EMAIL)
      ? email
      : 'cliente@benzaburguer.com.br';

    const body = {
      items: [{
        id: 'BENZA-' + Date.now(),
        title: String(produto || "Benza Burguer").substring(0, 256),
        description: 'Pedido Benza Burguer',
        category_id: 'food',
        quantity: 1,
        currency_id: 'BRL',
      ...
      payer: {
        ...
        surname: String(nome || 'Cliente').split(' ').slice(1).join(' ') || 'Benza',
        email: payerEmail
      },
      back_urls: {
      ...
      statement_descriptor: "BENZA BURGUER",
      external_reference: 'BENZA-' + Date.now(),
      expires: false
    };
